...
namespace Finance.Tests.PriceEval
{
    public class PriceEvaluationAppService_Tests : FinanceTestBase
    {
        ...
        [Fact]
        public async Task InternalPriceEvaluationStart_Test()
        {
            // Act
            var output = await _priceEvaluationAppService.PriceEvaluationStart(new PriceEvaluationStartInput
            {
                ...
                ModelCount = new List<CreateModelCountDto> { new CreateModelCountDto { Order = 1, Product = "123", ProductType = "123", Pixel = "123",  ModelCountYearList = new List<CreateModelCountYearDto> { new CreateModelCountYearDto { Year = 2023, UpDown = YearType.Year, Quantity = 1 } } } },
