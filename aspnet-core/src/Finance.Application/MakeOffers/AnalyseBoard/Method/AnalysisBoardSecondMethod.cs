...
public class AnalysisBoardSecondMethod : AbpServiceBase, ISingletonDependency
{
    ...
    /// <summary>
    /// 报价分析看板、报价反馈第一次查询接口0 报价分析看板，1 报价反馈
    /// </summary>
    /// <returns></returns>
        ...
        AnalyseBoardSecondInputDto analyseBoardSecondInputDto)
    {
        ...
        if ("PriceEvalType_Sample".Equals(pricetype))
        {
            ...
            analyseBoardSecondDto.ntime = ntime;
...
//根据毛利率，计算值
        foreach (var gro in gross)
        {
            ...
            foreach (var modelCountDto in modelcoutnlist)
            {
                ...
                decimal njl = 1; //年降率
                foreach (var mc in mcyl) //模组数量合计每年的的值
                {
                    long gradientid = GetGradient(mc.Quantity, gradients);
                    var sop = sops.FindFirst(p =>
                        p.Product.Equals(modelCountDto.Product) &&
                        p.GradientId == gradientid); //sop是之前计算出来的sop单价表，根据对应方案和梯度。找到对应毛利和单价list

                    var dj = sop.GrossValues.FindFirst(p => p.Gross.Equals(gro.ToString()))
                        .Grossvalue; //根据毛利率找到对应单价
                    ...
                    zsr += njl * dj * mc.Quantity * (1 - createRequirementDto.OneTimeDiscountRate / 100) *
                           (1 - createRequirementDto.AnnualRebateRequirements / 100); //总收入
                    yj += njl * dj * mc.Quantity * (createRequirementDto.CommissionRate / 100); //佣金
                }
            }
            ...
            ml.GrossMargin = gro;
            ml.GrossMarginNumber = zsr - cost - yj; //销售毛利 销售收入-销售成本-佣金
            xsmls.Add(ml);
            GrossMarginModel mll = new GrossMarginModel();
            mll.GrossMargin = gro;
            mll.GrossMarginNumber = (ml.GrossMarginNumber / zsr) * 100; //毛利率 销售毛利/销售收入
            mlls.Add(mll);
        }
        ...
        foreach (var gradient in gradients)
        {
            ...
            //用于目标内部
            decimal mbnbsl = 0; //数量
            decimal mbnbxscb = 0; //销售成本
            decimal mbnbxssr = 0; //销售收入
            decimal mbnbml = 0; //毛利
            decimal mbnbyj = 0; //佣金
//目标客户
            decimal mbkhsl = 0; //数量
            decimal mbkhxscb = 0; //销售成本
            decimal mbkhxssr = 0; //销售收入
            decimal mbkhml = 0; //毛利
            decimal mbkhyj = 0; //佣金
            foreach (var solution in Solutions)
            {
...
//齐套相关
    ...
    public async Task<AnalyseBoardSecondDto> getStatementAnalysisBoardSecond(long auditFlowId, int version, int ntype)
    {
        ...
        analyseBoardSecondDto.ntype = ntype;
        analyseBoardSecondDto.ntime = solutionQuotation.ntime;
        return analyseBoardSecondDto;
    }
    ...
    {
        //本流程下，非本版本下的方案组合
        var sols = await _solutionQutation.GetAllListAsync(p =>
            p.AuditFlowId == AuditFlowId && p.ntime == ntime && p.version != version);
        var idLists = solutions.OrderBy(p => p.Id).Select(p => p.Id).ToList();
        var idss = string.Join(",", idLists);

        foreach (var sol in sols)
        {
            var solutonss = JsonConvert.DeserializeObject<List<Solution>>(sol.SolutionListJson);
            var idList = solutonss.OrderBy(p => p.Id).Select(p => p.Id).ToList();
...
//根据流程类型，对数据进行防呆处理
        ...
        foreach (var d in dymap)
        {
            ...
            foreach (var ds in d)
            {
                QuotedGrossMarginActual gross = new()
                {
                    ...
                    LastRoundPrice = ds.LastRoundPrice,
                    LastRoundGrossMargin = ds.LastRoundGrossMargin,
                    LastRoundClientGrossMargin = ds.LastRoundClientGrossMargin,
                    LastRoundNreGrossMargin = ds.LastRoundNreGrossMargin,
        ...
        foreach (var grossMargin in grossMarginProjectModels)
        {
            ...
            var dys = (from gross in grossMargin.QuotedGrossMarginActualList
                select new DynamicUnitPriceOffers()
                {
                    ...
                    LastRoundPrice = gross.LastRoundPrice,
                    LastRoundGrossMargin = gross.LastRoundGrossMargin,
                    LastRoundClientGrossMargin = gross.LastRoundClientGrossMargin,
                    LastRoundNreGrossMargin = gross.LastRoundNreGrossMargin,
        ...
        int version, int ntype)
    {
        ...
        var gradientQuotedGross = (from jtsl in jtsls
            select new GradientGrossMarginCalculateModel()
            {
                ...
                LastRoundPrice = jtsl.LastRoundPrice,
                LastRoundGrossMargin = jtsl.LastRoundGrossMargin,
                LastRoundClientGrossMargin = jtsl.LastRoundClientGrossMargin,
                LastRoundNreGrossMargin = jtsl.LastRoundNreGrossMargin,
        ...
        List<GradientGrossMarginCalculateModel> gradientQuotedGross, int version, int ntype)
    {
        var list = (from gradientQuotedGro in gradientQuotedGross
            select new GradientGrossCalculate()
            {
                ...
                LastRoundPrice = gradientQuotedGro.LastRoundPrice,
                LastRoundGrossMargin = gradientQuotedGro.LastRoundGrossMargin,
                LastRoundClientGrossMargin = gradientQuotedGro.LastRoundClientGrossMargin,
                LastRoundNreGrossMargin = gradientQuotedGro.LastRoundNreGrossMargin,
    ...
    {
        var boards = await _resourceProjectBoardSecondOffers.GetAllListAsync(p =>
            p.AuditFlowId == AuditFlowId && p.version == version && p.ntype == ntype);
        var ntime = _solutionQutation.FirstOrDefault(p => p.AuditFlowId == AuditFlowId && p.version == version).ntime;
        var last = ntime - 1;
        List<BoardModel> boardModels = new();
    ...
    {
        List<PooledAnalysisOffers> pools = await _resourcePooledAnalysisOffers.GetAllListAsync(p =>
            p.AuditFlowId == auditFlowId && p.version == version && p.ntype == ntype);
    ...
    {
        List<UnitPriceOffers> ups = await _resourceUnitPriceOffers.GetAllListAsync(p =>
            p.AuditFlowId == auditFlowId && p.version == version && p.ntype == ntype);
        ...
        string fileName)
    {
        ...
        {
            templatePath = "wwwroot/Excel/报价审批表模板—不含样品.xlsx";
        }
...
//客户目标价
    ...
    public async Task<List<NreExcel>> getNreForExcel(long processId, int version, int ntype)
    {
        ...
        List<NreQuotation> nreQuotations = await _nreQuotation.GetAllListAsync
            (p => p.AuditFlowId == processId && p.version == version && p.ntype == ntype);
    ...
    public async Task<List<AnalyseBoardNreDto>> getNreForData(long processId, int version, int ntype)
    {
        ...
        List<DeviceQuotation> deviceQuotations =
                await _deviceQuotation.GetAllListAsync(p =>
                    p.AuditFlowId == processId && p.version == version && p.ntype == ntype)
            ;
        List<NreQuotation> nreQuotations = await _nreQuotation.GetAllListAsync(p =>
            p.AuditFlowId == processId && p.version == version && p.ntype == ntype);
        if (nreQuotations is null || nreQuotations.Count == 0)
        {
            return nres;
    ...
    /// <summary>
    /// 获取是否生成报价反馈  true就是有反馈，false相反
    /// </summary>
    /// <param name="departmentDtos"></param>
    /// <returns></returns>
    public async Task<bool> getQuotation(long auditFlowId, int version)
    {
        var solutionQuotation = await _solutionQutation.FirstOrDefaultAsync(p =>
            p.AuditFlowId == auditFlowId && p.version == version);

        var status = solutionQuotation.status;
        if (status == 1)
        {
            return true;
        }
    ...
    public async Task<ManagerApprovalOfferDto> GetManagerApprovalOfferOne(long processId, int version, int ntype)
    {
        ...
        if ("PriceEvalType_Sample".Equals(priceEvaluationStartInputResult.PriceEvalType))
        {
            ...
            managerApprovalOfferDto.SampleOffer = samples;
            managerApprovalOfferDto.issample = true;
    ...
    public async Task<QuotationListSecondDto> GetManagerApprovalOfferTwo(long processId, int version, int ntype)
    {
        ...
        AuditQuotationListSave quotationListSave = await _financeAuditQuotationListSave.FirstOrDefaultAsync(p =>
            p.AuditFlowId.Equals(processId) && p.version.Equals(version) && p.nsource.Equals(0));

        if (quotationListSave != null)
        {
            string auditQuotationListJson = quotationListSave.AuditQuotationListJson;
            if (auditQuotationListJson is not null)
            {
                var excl = JsonConvert.DeserializeObject<ExcelApprovalDto>(auditQuotationListJson);
                pp.ExchangeRate = excl.exchangeRate; //汇率
            }
    ...
    public async Task PostQuotationApprovedSave(ExcelApprovalDtoSave quotationListSecondDtoSave)
    {
        ...
        await _financeAuditQuotationListSave.HardDeleteAsync(p =>
    ...
    internal async Task<List<long>> GetExternalQuotationNumberOfQuotations(long auditFlowId, long solutionId)
    {
        ...
        long ii = await _externalQuotation.CountAsync(p =>
            p.AuditFlowId.Equals(auditFlowId) && p.IsSubmit &&
            solutionQuotations.Select(p => p.Id).Contains(p.SolutionId));
        ...
        long numberOfQuotations)
    {
        ...
        if (externalQuotationDto.ProductQuotationListDtos is null)
            externalQuotationDto.ProductQuotationListDtos = new();
