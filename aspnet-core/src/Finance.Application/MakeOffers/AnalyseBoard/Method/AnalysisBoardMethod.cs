...
namespace Finance.MakeOffers.AnalyseBoard.Method
{
    ...
    public class AnalysisBoardMethod : AbpServiceBase, ISingletonDependency
    {
        ...
        /// Nre 品保录入 实验项目 实体类
        /// </summary>
        private readonly IRepository<EnvironmentalExperimentFee, long> _resourceQADepartmentTest;
        /// <summary>
        /// Nre 项目管理部 差旅费实体类
        /// </summary>
        ...
        public AnalysisBoardMethod(
            ...
            IRepository<EnvironmentalExperimentFee, long> qADepartmentTest,
        ...
        public async Task<List<UnitPriceModel>> CalculateUnitPriceModel(List<UnitPriceCountModel> unitPrice, long? GrossMarginId)
        {
            ...
            foreach (var unitPriceItem in unitPrice)
            {
                ...
                unitPriceModel.ProductId = unitPriceItem.ProductId;
        ...
        public async Task<ProductBoardDtoOrEvery> BoardTwoFrom(List<UnitPriceCountModel> unitPrice, long processId)
        {
            ...
            foreach (UnitPriceCountModel unit in unitPrice)
            {
                ...
                productBoardModel.ProductId = unit.ProductId;//模组数量主键
                //报价分析看板中的 动态单价表 
                DynamicUnitPriceOffers dynamicUnitPriceOffer = await _resourceDynamicUnitPriceOffers.FirstOrDefaultAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(productBoardModel.ProductId));
                ...
                InteriorUnitPrice.Add(new DynamicProductBoardModel() { ProductId = unit.ProductId, UnitPrice = unit.Unit / (1 - GrossMargin) });
                var prop = (from a in await _resourceModelCount.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.Id.Equals(unit.ProductId))
                ...
                {
                    //售价(单价)
                    var sellingPrice = await SellingPrice(processId, productBoardModel.ProductId, 1M, item, lastYearUnitPrice, 0.0M);
                    lastYearUnitPrice = sellingPrice;
                    decimal RebateSalesRevenueValue = await RebateSalesRevenue(processId, productBoardModel.ProductId, 0.0M, item, lastYearUnitPrice, lastYearUnitPrice);
                    //返利后销售收入与收入
                    AllRebateSalesRevenue += RebateSalesRevenueValue;
                    AllTradingProfit += await TradingProfit(processId, productBoardModel.ProductId, 0.0M, item, 0.0M, lastYearUnitPrice);
                }
                #endregion
                #region 本次报价       
                ...
                if (dynamicUnitPriceOffer is not null)
                {
                    ...
                    OfferUnitPrice.Add(new DynamicProductBoardModel() { UnitPrice = dynamicUnitPriceOffer.OfferUnitPrice, ProductId = unit.ProductId });
                    decimal OfferlastYearUnitPrice = dynamicUnitPriceOffer.OfferUnitPrice;
                    //循环所有的年份               
                    foreach (int item in yearList)
                    {
                        //售价(单价)
                        var sellingPrice = await SellingPrice(processId, productBoardModel.ProductId, 1M, item, OfferlastYearUnitPrice, 0.0M);
                        OfferlastYearUnitPrice = sellingPrice;
                        decimal RebateSalesRevenueValue = await RebateSalesRevenue(processId, productBoardModel.ProductId, 0.0M, item, OfferlastYearUnitPrice, OfferlastYearUnitPrice);
                        //返利后销售收入与收入
                        AllOfferRebateSalesRevenue += RebateSalesRevenueValue;
                        AllOfferTradingProfit += await TradingProfit(processId, productBoardModel.ProductId, 0.0M, item, 0.0M, OfferlastYearUnitPrice);
                    }
                }
                #endregion
        ...
        public async Task<ProductBoardGrossMarginDto> CalculateFullGrossMargin(ProductBoardProcessDto productBoardDtos)
        {
            ...
            foreach (DynamicProductBoardModel productBoardModel in productBoardDtos.ProductBoards)
            {
                ...
                {
                    //售价(单价)
                    var sellingPrice = await SellingPrice(productBoardDtos.AuditFlowId, productBoardModel.ProductId, 1M, item, lastYearUnitPrice, 0.0M);
                    lastYearUnitPrice = sellingPrice;
                    decimal RebateSalesRevenueValue = await RebateSalesRevenue(productBoardDtos.AuditFlowId, productBoardModel.ProductId, 0.0M, item, lastYearUnitPrice, lastYearUnitPrice);
                    //返利后销售收入与收入
                    AllRebateSalesRevenue += RebateSalesRevenueValue;
                    decimal TradingProfitValue = await TradingProfit(productBoardDtos.AuditFlowId, productBoardModel.ProductId, 0.0M, item, 0.0M, lastYearUnitPrice);
                    //单年份的销售毛利
        ...
        public async Task<List<YearDimensionalityComparisonDto>> YearDimensionalityComparison(long processId, decimal grossMargin, List<DynamicProductBoardModel> dynamicProductBoardModels)
        {
            ...
            //年份
            List<int> YearList = await GetYear(processId);
            List<long> AllProductId = dynamicProductBoardModels.Select(p=>p.ProductId).ToList();
            ...
            foreach (var Year in YearList)
            {
                ...
                {                  
                   sopOrValueMode.Value += await SalesQuantity(processId, boardModel.ProductId, Year);
                }                
            ...
            foreach (var Year in YearList)
            {
                ...
                foreach (var ProductId in AllProductId)//循环每一个模组
                {
                    ...
                    ModelCountYear modelCountYear = await _resourceModelCountYear.FirstOrDefaultAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(ProductId) && p.Year.Equals(Year));
                    PriceEvaluationTableDto priceEvaluationTableDto = new();
            ...
            foreach (var Year in YearList)
            {
                ...
                {
                    //上一年的单价
                    if (!DictiolastYearUnitPrice.ContainsKey(item.ProductId)) DictiolastYearUnitPrice[item.ProductId] = item.UnitPrice;
                    decimal ALLYearPrice = await SellingPrice(processId, item.ProductId, 1M, Year, DictiolastYearUnitPrice[item.ProductId], 0.0M);
                    DictiolastYearUnitPrice[item.ProductId] = ALLYearPrice;
                    if (!DictiolastYearUnitPrice.ContainsKey(item.ProductId)) DictiolastYearUnitPrice[item.ProductId] = item.UnitPrice;
                    //销售数量
                    decimal Count = await SalesQuantity(processId, item.ProductId, Year);
            ...
            foreach (var Year in YearList)
            {
                ...
                {
                    //上一年的单价
                    if (!DictiolastYearUnitPrice.ContainsKey(dynamicProductBoardModel.ProductId)) DictiolastYearUnitPrice[dynamicProductBoardModel.ProductId] = dynamicProductBoardModel.UnitPrice;
                    decimal ALLYearPrice = await SellingPrice(processId, dynamicProductBoardModel.ProductId, 1M, Year, DictiolastYearUnitPrice[dynamicProductBoardModel.ProductId], 0.0M);
                    ...
                    decimal InteriorALLincome = ALLYearPrice * Count;
                    //某一年 某一个模组 返利金额
                    decimal TargetGrossMarginALLRebateMoney = await SomeModelCountRebateMoneyYear(processId, dynamicProductBoardModel.ProductId, Year, 1M, InteriorALLincome);
                    //该 毛利率 全生命周期的税费损失
                    decimal GrossMarginALLRebateMoney = await SomeModelCountAddTaxRateYear(TargetGrossMarginALLRebateMoney, Year);
                    //返利后销售成本
                    decimal unitCost = 0M;
                    PriceEvaluationTableDto priceEvaluationTableDto = new();
                    ModelCountYear modelCountYear = await _resourceModelCountYear.FirstOrDefaultAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(dynamicProductBoardModel.ProductId) && p.Year.Equals(Year));
                    if (!string.IsNullOrWhiteSpace(modelCountYear.TableJson)) priceEvaluationTableDto = JsonConvert.DeserializeObject<PriceEvaluationTableDto>(modelCountYear.TableJson);
                    if (priceEvaluationTableDto.TotalCost is not 0.0M) unitCost = priceEvaluationTableDto.TotalCost;
        ...
        ///获取总共 零件
        /// </summary>
        private async Task<List<SolutionModel>> PartDtoList(long processId)
        {
            //根据流程表主键取模组数量
            List<ModelCount> modelCount = await _resourceModelCount.GetAllListAsync(p => p.AuditFlowId.Equals(processId));
            List<SolutionModel> partModel = (from a in modelCount
                                         select new SolutionModel
                                         {
                                             SolutionId = a.Id,
                                             SolutionName = a.Product,
                                         }).ToList();
            return partModel;
        }
        /// <summary>
        ...
        public async Task<QuotationListDto> QuotationList(long processId)
        {
            try
            {
                ...
                quotationListDto.MotionMessage.Add(new MotionMessageModel() { MessageName = motionMessageModel.MessageName, Sop = motionMessageModel.Sop });
                #endregion
                List<SolutionModel> partModels = await PartDtoList(processId);
                //核心部件
                #region 核心部件
                ...
                foreach (SolutionModel partModel in partModels)
                {
                    ...
                    coreComponentModel.ProductId = partModel.SolutionId;
                    coreComponentModel.ComponentName = partModel.SolutionName;
                    List<ElectronicBomInfo> electronicBomInfos = await _resourceElectronicBomInfo.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(partModel.SolutionId));
                    var SensorTypeSelect = (from a in await _resourceModelCount.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.Id.Equals(partModel.SolutionId))
                                            join b in await _resourceProductInformation.GetAllListAsync(p => p.AuditFlowId.Equals(processId)) on a.Product equals b.Product
                                            select new
                                            {
                    ...
                    var IspTypeSelect = (from a in await _resourceModelCount.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.Id.Equals(partModel.SolutionId))
                                         join b in await _resourceProductInformation.GetAllListAsync(p => p.AuditFlowId.Equals(processId)) on a.Product equals b.Product
                                         select new
                                         {
                    ...
                    var SerialChipTypeSelect = (from a in await _resourceModelCount.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.Id.Equals(partModel.SolutionId))
                                                join b in await _resourceProductInformation.GetAllListAsync(p => p.AuditFlowId.Equals(processId)) on a.Product equals b.Product
                                                select new
                                                {
                    ...
                    var LedTypeSelect = (from a in await _resourceModelCount.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.Id.Equals(partModel.SolutionId))
                                         join b in await _resourceProductInformation.GetAllListAsync(p => p.AuditFlowId.Equals(processId)) on a.Product equals b.Product
                                         select new
                                         {
                    ...
                    List<StructureBomInfo> structureBomInfos = await _resourceStructureBomInfo.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(partModel.SolutionId));
                    ...
                    var LensTypeSelect = (from a in await _resourceModelCount.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.Id.Equals(partModel.SolutionId))
                                          join b in await _resourceProductInformation.GetAllListAsync(p => p.AuditFlowId.Equals(processId)) on a.Product equals b.Product
                                          select new
                                          {
                    ...
                    var CableTypeSelect = (from a in await _resourceModelCount.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.Id.Equals(partModel.SolutionId))
                                           join b in await _resourceProductInformation.GetAllListAsync(p => p.AuditFlowId.Equals(processId)) on a.Product equals b.Product
                                           select new
                                           {
                ...
                foreach (SolutionModel partModel in partModels)
                {
                    ...
                    nreCostMessageModel.ProductId = partModel.SolutionId;
                    nreCostMessageModel.NreCostModuleName = partModel.SolutionName;
                    //手板件费
                    nreCostMessageModel.NreCostModels = new();
                    List<HandPieceCost> handPieceCosts = await _resourceHandPieceCost.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(partModel.SolutionId));
                    ...
                    //模具费
                    List<MouldInventory> mouldInventories = await _resourceMouldInventory.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.SolutionId.Equals(partModel.SolutionId));
                    ...
                    //检具费
                    List<QADepartmentQC> qADepartmentQCs = await _resourceQADepartmentQC.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(partModel.SolutionId));
                    ...
                    //生产设备费用 
                    List<EquipmentInfo> equipmentInfos = (from a in await _resourceWorkingHoursInfo.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(partModel.SolutionId))
                    ...
                    List<WorkingHoursInfo> workingHours = await _resourceWorkingHoursInfo.GetAllListAsync(p => p.AuditFlowId.Equals(processId)&&p.ProductId.Equals(partModel.SolutionId));
                    //==>测试线费用+工装费用
                    ...
                    //==>-产品部-电子工程师录入的试验费用
                    List<LaboratoryFee> laboratoryFees = await _resourceLaboratoryFee.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(partModel.SolutionId));
                    //==>-品保部录入的实验费用
                    List<EnvironmentalExperimentFee> qADepartmentTests = await _resourceQADepartmentTest.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.SolutionId.Equals(partModel.SolutionId));
                    ...
                    List<WorkingHoursInfo> workingHoursInfos = await _resourceWorkingHoursInfo.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(partModel.SolutionId));
                    ...
                    //研发费(差旅费)
                    List<TravelExpense> travelExpenses = await _resourceTravelExpense.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(partModel.SolutionId));
                    ...
                    //其他费用
                    List<RestsCost> rests = await _resourceRestsCost.GetAllListAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(partModel.SolutionId));
                ...
                foreach (SolutionModel partModel in partModels)
                {
                    ModelCount priceEvaluationTableDtos = await _resourceModelCount.FirstOrDefaultAsync(p => p.AuditFlowId.Equals(processId) && p.Id.Equals(partModel.SolutionId));
                    PriceEvaluationTableDto priceEvaluationTableDto = new();
                    ...
                    var CostValue = 0.0M;
                    pricingMessageModel.ProductId = partModel.SolutionId;
                    pricingMessageModel.PricingMessageName = partModel.SolutionName;
                ...
                foreach (SolutionModel partModel in partModels)
                {
                    BiddingStrategy biddingStrategy = await _financeBiddingStrategy.FirstOrDefaultAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(partModel.SolutionId));
                    if (biddingStrategy is null) biddingStrategy = new();
                    ModelCount modelCount = new();
                    modelCount = await _resourceModelCount.FirstOrDefaultAsync(p => p.AuditFlowId.Equals(processId) && p.Id.Equals(partModel.SolutionId));
                    PriceEvaluationTableDto priceEvaluationTableDto = new();
                    ...
                    dynamicUnitPriceOffers = await _resourceDynamicUnitPriceOffers.FirstOrDefaultAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(partModel.SolutionId));
                    if (dynamicUnitPriceOffers is null) dynamicUnitPriceOffers = new();
                    biddingStrategyModels.Add(new BiddingStrategyModel() { ProductId = partModel.SolutionId, Product = partModel.SolutionName, Cost = priceEvaluationTableDto.TotalCost, GrossMargin = dynamicUnitPriceOffers.OffeGrossMargin, Price = dynamicUnitPriceOffers.OfferUnitPrice, Commission = biddingStrategy.Commission, GrossMarginCommission = biddingStrategy .GrossMarginCommission});
                }
        ...
        private async Task<decimal> SalesQuantity(long processId, long partId, int year)
        {
            ...
            else
            {
                ...
                modelCount = await _resourceModelCountYear.FirstOrDefaultAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(partId) && p.Year.Equals(year));
            }
        ...
        private async Task<decimal> SellingPrice(long processId, long partId, decimal grossMargin, int year, decimal lastYearUnitPrice, decimal YearUnitPrice)
        {
            ...
            if (lastYearUnitPrice is 0.0M)//如果不传上一年的单价  就证明 是获取今年的单价  
            {
                ...
                else
                {
                    ...
                    ModelCountYear modelCountYear = await _resourceModelCountYear.FirstOrDefaultAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(partId) && p.Year.Equals(year));
                    PriceEvaluationTableDto priceEvaluationTableDto = new();
        ...
        {
            List<long> AllProductId = _resourceModelCount.GetAllList(p => p.AuditFlowId.Equals(processId)).Select(p => p.Id).ToList();
            decimal AllTotalCost = 0.0M;
            foreach (var ProductId in AllProductId)
            {
                ModelCountYear modelCountYear = await _resourceModelCountYear.FirstOrDefaultAsync(p => p.AuditFlowId.Equals(processId) && p.ProductId.Equals(ProductId) && p.Year == year);
                PriceEvaluationTableDto priceEvaluationTableDto = new();
                if (modelCountYear is not null && !string.IsNullOrWhiteSpace(modelCountYear.TableJson)) priceEvaluationTableDto = JsonConvert.DeserializeObject<PriceEvaluationTableDto>(modelCountYear.TableJson);
        ...
        private async Task<decimal> Allincome(long processId, decimal grossMargin)
        {
            ...
            foreach (var ProductId in AllProductId)//循环每一个模组
            {
                ...
                foreach (var Year in YearList)
                {
                    ...
                    //decimal lastYearUnitPrice = await SellingPrice(processId, ModelCount, grossMargin, Year-1, 0.0M, 0.0M);
                    //每一年的单价
                    decimal ALLYearPrice = await SellingPrice(processId, ProductId, grossMargin, Year, lastYearUnitPrice, 0.0M);
        ...
        private async Task<decimal> AllPrice(long processId, decimal grossMargin)
        {
            ...
            foreach (var Year in YearList)
            {
                foreach (var ModelCount in AllProductId)//循环每一个模组
                {
                    //上一年的单价
                    decimal lastYearUnitPrice = await SellingPrice(processId, ModelCount, grossMargin, Year - 1, 0.0M, 0.0M);
                    //每一年的单价
